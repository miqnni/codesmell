      ...
      <Flex h={16} alignItems={"center"} justifyContent={"space-between"}>
        ...
        <HStack alignItems={"center"}>
          ...
          <HStack as={"nav"} display={{ base: "none", md: "flex" }}>
            {Links.map((link) => (
              <NavLink key={link.name} simpleLink={link}>
                {link.name}
                {link.external ? <LuExternalLink /> : null}
              </NavLink>
            ))}
          ...
          <Menu.Root positioning={{ placement: "right-end" }}>
            <Menu.Trigger rounded="full" focusRing="outside">
              <Avatar.Root size="sm">
                ...
                {isLoggedIn ? (
                  <Avatar.Image src="https://f4.bcbits.com/img/a2506847905_10.jpg" />
                ) : (
                  <Avatar.Image src="https://upload.wikimedia.org/wikipedia/en/c/cc/Wojak_cropped.jpg" />
                )}
              </Avatar.Root>
            </Menu.Trigger>
            <Portal>
